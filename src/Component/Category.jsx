  ...
  const handleAddCategory = async ()=>{
    if(categoryName){
      ...
      if(response.status>=200 && response.status<300){
        ...
        toast.error("Operation failes!!!Please try after some time....")
      }
    }else{
       toast.warning("Plesae provide category name!!!")
    }
  }
  ...
  return (
    ...
    </div>
    {
      allCategories.length>0?allCategories?.map(item=>(
         <div className="mt-5 ms-3 border rounded p-3" droppable onDragOver={(e)=>dragOver(e)} onDrop={(e)=>videoDrop(e,item?.id)}  >
               <div className="d-flex justify-content-between align-itewms-center">
